...
def shipmentVolumePackages(packages: list[int]) -> int:
    ...
    while (first_package := package_queue.dequeue()) is not None and (
        ...
        back_package = package_queue.peek_back()
        if back_package is None or back_package <= new_package:
            package_queue.enqueue(new_package)
            continue

        # 末尾以外の場所に荷物を追加する
        # 追加できる場所を探す
        while (
            front_package := package_queue.peek_front()
        ) is not None and front_package <= new_package:
            package_queue.dequeue()
            package_queue.enqueue(front_package)
        ...
        # 最も小さい荷物を先頭に昇順とする
        while (
            (front_package := package_queue.peek_front()) is not None
            and (back_package := package_queue.peek_back()) is not None
        ) and front_package >= back_package:
            package_queue.dequeue()
            package_queue.enqueue(front_package)
